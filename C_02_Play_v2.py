...
def get_study_names():
    ...
    while len(study_names) < 4:
        ...
        if potential_study[1] not in study_types:
            study_types.append(potential_study[1])
...
class Play:
    ...
    def __init__(self, how_many, mode):
        ...
        control_button_list = [
            [self.game_frame, "Next Round", "#86a5c4", self.new_round, 21, 5, None],
    ...
    def new_round(self):
        ...
        for count, item in enumerate(self.answer_button_ref):
            item.config(text=study_names[count], state=NORMAL)
        ...
        round_question = study_types[self.round_number_ref]
        round_answer = study_names[self.round_number_ref]
        print(round_answer)
        self.question_label.config(text=f"What is the {round_question}?")
