...
# get the studies from the csv file
def get_studies():
    """
    ...
    # return the data from the csv for the program to use
    return all_studies


# gets the names and study types for round
def get_study_names(mode):
    """
    Choose three studies from larger list ensuring that they're all different
    only three studies as the forth (and correct) answer comes form the question function
    ...
    # set up empty lists for the data to be put into
    study_types = []
    answers = []
    ...
    while len(answers) < 3:
        ...
        # append to answers
        # still use study types list to check
        if potential_study[1] not in study_types:
            if mode == "second":
                ...
                answers.append(actual_rad_study)

            # append the study name to the list for the round
            else:
                answers.append(potential_study[0])

    return answers


# get the question and answer for the round
def round_question(mode):
    """
    Choose a question and its answer from csv
    ...
    # get the study type and check it hasn't already been asked
    # also different data for the different modes
    if mode == "first":
...
# opens text file and create lists for leaderboard
def open_file():
    """
    Opens text file that leaderboard scores are kept on
    """

    # filename for the text file
    file_name = "previous_top_scores.txt"
    try:
        # open the existing file and read the data
        with open(file_name, "r") as file:
            result = file.read().split()
            # split into the two bits of data needed
            names = result[::2]
            points = result[1::2]
...
class StartGame:
    ...
    def __init__(self):
        ...
        # make the button to view the leaderboard
    ...
    def check_rounds(self, mode):
        """
        checks that 1 or more rounds [no decimals] have been entered
        """

        # get the amount of rounds entry
        ...
        # set up strings for error
        error = "Please enter a whole number more than 0"
        error_check = "Nope"

        # appropriate amount of rounds checker (appropriate = whole positive int)
        try:
            ...
            # inappropriate amount of rounds
            else:
                error_check = "yes"
        # wrong values
        except ValueError:
    ...
    def to_end(self):
        ...
        """
        score = 0
        # mode_type helps to ensure smooth experience
        mode_type = "start"
        Leaderboard(self, score, mode_type)
...
class Play:
    ...
    def __init__(self, how_many, mode):
        ...
        # defining for when getting questions
        self.warning_label = Label(self.game_frame, text="This is warning")
        ...
        play_labels_ref = []
        # make the labels and append them to the list
    ...
    def new_round(self, mode):
        """
        Choose three studies and configures them onto the buttons.
        """
        ...
        # the question and correct answer for this round
        question, answer = round_question(mode)

        # get round studies and their names
        actual_answers = get_study_names(mode)

        # list of questions asked so far
        questions_so_far = self.round_questions

        # do a test for this
        # if questions asked so far reach the max amount
        if len(questions_so_far) == 2:
            # clear list, add warning label
            questions_so_far.clear()
            self.warning_label.config(text="You have answered all questions, they are now repeating",
                                      font=("Arial", "13"), bg="#F8CECC")
            self.warning_label.grid(row=6)
        # if not show no label
        else:
            self.warning_label.config(text="", font=("Arial", "13"), bg="#F0F0F0")
            self.warning_label.place_forget()

        # is question has already been asked, grab a new one
        ...
        # if there is duplicate answers on buttons get new ones
        if answer in actual_answers:
            while answer in actual_answers:
                actual_answers = get_study_names(mode)
        else:
            pass
        ...
        actual_answers.insert(self.input_place, answer)
        round_questions = actual_answers

        # make questions for this round
        if mode == "first":
        ...
        # configure the answer buttons to display the answers
    ...
    def get_score(self, user_choice):
        ...
        # if user chooses correct answer add one to score
        ...
        # change colour of users choice to reflect incorrect answer
        number = int(user_choice)
        self.answer_button_ref[number].config(bg="#e0645e")

        # change colour of users choice to reflect correct answer
        number = int(self.input_place)
        self.answer_button_ref[number].config(bg="#42d442")

    # close play and go back to start
    def close_play(self):
...
class DisplayHints:
    ...
    def __init__(self, partner):
        ...
        # disable hint and end game button(prevent errors)
        ...
        # set up text to be displayed to user
        ...
        # make the label for hint_text to display it
...
class Stats:
    ...
    def __init__(self, partner, all_stats_info):
        ...
        self.stats_frame = Frame(self.stats_box, width=300, bg="#bdeaff")
        ...
        # custom comment text and formatting based on users score
        ...
        # make the labels for stats to display information
        ...
        # display leaderboard highest winner
        # if no entries in leaderboard ignore
...
class Leaderboard:
    ...
    def __init__(self, partner, score, mode_type):
        self.end_box = Toplevel()

        # make frame for leaderboard
        self.end_frame = Frame(self.end_box)
        self.end_frame.grid(padx=10, pady=10)

        # If users press cross at top
        # if user coming from start screen bring them back to start
        if mode_type == "start":
            self.end_box.protocol('WM_DELETE_WINDOW',
                                  lambda: [self.back_to_start(partner, mode_type)])
        # if user coming from end screen destroy the leaderboard
        else:
            self.end_box.protocol('WM_DELETE_WINDOW', root.destroy)

        # disable buttons on the start screen if user comes from start
        if mode_type == "start":
            partner.first_mode_button.config(state=DISABLED)
            partner.second_mode_button.config(state=DISABLED)
            partner.leaderboard_button.config(state=DISABLED)
        else:
            pass
        ...
        if length_leaderboard == 0:
            ...
            # if there is actually already score take the lowest score value
        ...
        # if user came from start or got a score of 0
        if mode_type == "start" or score == 0:
            pass

        # if user has a higher score than low point or the leaderboard has left than 3 entries
        ...
        self.play_again = Button(self.end_frame, text="PLay Again", bg="#cc81a5",
                                 command=lambda: [self.back_to_start(partner, mode_type)], font=("Arial", "15"),
                                 fg="#000000", width=21)
        self.play_again.grid(row=7, pady=5, padx=5)

        # if user came from start menu, change play again to say main menu
        if mode_type == "start":
            self.play_again.config(text="Main Menu")

    def check_name(self, score):
        name_entry = self.winners_name.get()

        # if the name entry is equal to 3
        if len(name_entry) == 3:

            # if name entry is only letters, accept entry and update leaderboard
            ...
            # if name entry contains things other than letters, reject and custom error message
            else:
                self.error_message.grid(row=2)
                self.error_message.config(text="Please enter do not enter numbers")

        # if error message is not exactly 3 characters, reject, custom error message
        else:
            self.error_message.grid(row=2)
    ...
    def update_leaderboard(self, score):
        ...
        # sort name list to be proper order
        ...
        # open file and write leaderboard entries to it
    ...
    def back_to_start(self, partner, mode_type):
        # reshow root (ie main menu) and end
        # current game / allow new game to start
        if mode_type == "start":
            partner.first_mode_button.config(state=NORMAL)
            partner.second_mode_button.config(state=NORMAL)
            partner.leaderboard_button.config(state=NORMAL)
        else:
            pass
        root.deiconify()
